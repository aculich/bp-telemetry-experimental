...
"""
Workspace-to-Database Mapper for Antigravity.
...
class AntigravityWorkspaceMapper:
    """
    ...
    def __init__(self, session_monitor):
        ...
        self.cache_file = Path.home() / ".blueplane" / "antigravity_workspace_db_cache.json"
        self._load_cache()
    ...
    async def _match_by_path_hash(self, workspace_path: str) -> Optional[Path]:
        ...
        Antigravity stores databases in workspaceStorage/{uuid}/state.vscdb
    ...
    def _discover_all_databases(self) -> List[Path]:
        """Discover all Antigravity database files."""
        databases = []

        for base_path in get_antigravity_database_paths():
